...
func listenAndServe(addr *sctp.SCTPAddr, handler NGAPHandler) {
	...
	for {
		...
		logger.NgapLog.Infof("[OCF] SCTP Accept from: %s", newConn.RemoteAddr().String())
...
func handleConnection(conn *sctp.SCTPConn, bufsize uint32, handler NGAPHandler) {
	defer func() {
		// if OCF call Stop(), then conn.Close() will return EBADF because conn has been closed inside Stop()
