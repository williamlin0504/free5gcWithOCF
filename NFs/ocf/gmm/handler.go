...
func HandleULNASTransport(ue *context.OcfUe, anType models.AccessType,
	...
	case nasMessage.PayloadContainerTypeUEPolicy:
		ue.GmmLog.Infoln("OCF Transfer UEPolicy To PCF")
	...
	case nasMessage.PayloadContainerTypeUEParameterUpdate:
		ue.GmmLog.Infoln("OCF Transfer UEParameterUpdate To UDM")
...
func transport5GSMMessage(ue *context.OcfUe, anType models.AccessType,
	ulNasTransport *nasMessage.ULNASTransport) error {
	...
	if ulNasTransport.OldPDUSessionID == nil {
		...
		// OCF has a PDU session routing context for the PDU session ID and the UE
		if smContextExist {
			...
			case nasMessage.ULNASTransportRequestTypeInitialRequest:
				...
				updateData := models.SmContextUpdateData{
					...
					SmContextStatusUri: fmt.Sprintf("%s/nocf-callback/v1/smContextStatus/%s/%d",
						ue.ServingOCF().GetIPv4Uri(), ue.Guti, pduSessionID),
				}
				...
				if err != nil {
					...
					if n2Info != nil {
						...
						case models.N2SmInfoType_PDU_RES_REL_CMD:
							ue.GmmLog.Debugln("OCF Transfer NGAP PDU Session Resource Release Command from SMF")
			...
			// case ii) OCF has a PDU session routing context, and Request type is "existing PDU session"
			...
			// other requestType: OCF forward the 5GSM message, and the PDU session ID IE towards the SMF identified
			// by the SMF ID of the PDU session routing context
			default:
				return forward5GSMMessageToSMF(ue, anType, pduSessionID, smContext, smMessage)
			}
		} else { // OCF does not have a PDU session routing context for the PDU session ID and the UE
			switch requestType.GetRequestTypeValue() {
			// case iii) if the OCF does not have a PDU session routing context for the PDU session ID and the UE
			...
			case nasMessage.ULNASTransportRequestTypeInitialRequest:
				...
				// A) OCF shall select an SMF

				// If the S-NSSAI IE is not included and the user's subscription context obtained from UDM. OCF shall
				...
				if ulNasTransport.DNN != nil {
					...
					// S-NSSAI, the OCF shall use a locally configured DNN as the DNN
					dnn = ue.ServingOCF().SupportDnnLists[0]
...
func forward5GSMMessageToSMF(
	...
	if err != nil {
		...
		if response.BinaryDataN2SmInformation != nil {
			...
			ue.GmmLog.Debugf("OCF forward Only N1 SM Message to UE")
...
func HandleRegistrationRequest(ue *context.OcfUe, anType models.AccessType, procedureCode int64,
	...
	case nasMessage.MobileIdentity5GSType5gGuti:
		...
		if reflect.DeepEqual(guamiFromUeGuti, servedGuami) {
			...
			ue.GmmLog.Debugf("Serving OCF has changed")
	...
	if ue.ServingOcfChanged {
		...
		if problemDetails != nil {
			if problemDetails.Cause == "INTEGRITY_CHECK_FAIL" || problemDetails.Cause == "CONTEXT_NOT_FOUND" {
				ue.GmmLog.Warnf("Can not retrieve UE Context from old OCF[Cause: %s]", problemDetails.Cause)
			} else {
...
func HandleInitialRegistration(ue *context.OcfUe, anType models.AccessType) error {
	...
	for {
		...
		if err != nil {
			ue.GmmLog.Error("OCF can not select an PCF by NRF")
		} else {
...
func getSubscribedNssai(ue *context.OcfUe) {
	...
	if ue.NudmSDMUri == "" {
		...
		for {
			...
			if err != nil {
				ue.GmmLog.Errorf("OCF can not select an Nudm_SDM Instance by NRF[Error: %+v]", err)
	...
	if len(ue.AllowedNssai[anType]) == 0 {
		for _, snssai := range ue.SubscribedNssai {
			if snssai.DefaultIndication {
				if ocfSelf.InPlmnSupportList(*snssai.SubscribedSnssai) {
					allowedSnssai := models.AllowedSnssai{
...
func AuthenticationProcedure(ue *context.OcfUe, accessType models.AccessType) (bool, error) {
	...
	if err != nil {
		ue.GmmLog.Error("OCF can not select an AUSF by NRF")
		return false, err
	}
...
func HandleAuthenticationResponse(ue *context.OcfUe, accessType models.AccessType,
	...
	case models.AuthType__5_G_AKA:
		...
		case models.AuthResult_SUCCESS:
			...
			ue.DerivateKocf()
			ue.GmmLog.Debugln("ue.DerivateKocf()", ue.Kocf)
