...
func HandleIKEAUTH(udpConn *net.UDPConn, ocfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	case PreSignalling:
		...
		if certificateRequest != nil {
			ikeLog.Info("UE request OCF certificate")
			if CompareRootCertificate(certificateRequest.CertificateEncoding, certificateRequest.CertificationAuthority) {
				responseCertificate =
					ike_message.BuildCertificate(ike_message.X509CertificateSignature, ocfSelf.OCFCertificate)
			}
		}
		...
		if responseCertificate == nil {
			responseCertificate = ike_message.BuildCertificate(ike_message.X509CertificateSignature, ocfSelf.OCFCertificate)
		}
		...
		signedAuth, err := rsa.SignPKCS1v15(rand.Reader, ocfSelf.OCFPrivateKey, crypto.SHA1, sha1HashFunction.Sum(nil))
		if err != nil {
	...
	case PostSignalling:
		...
		if authentication != nil {
			...
			// TODO: Finish authentication test for UE and OCF
			/*
		...
		if thisUE.TemporaryPDUSessionSetupData != nil {
			for {
				if len(thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
					...
					ikeSecurityAssociation := thisUE.OCFIKESecurityAssociation
					...
					if err := EncryptProcedure(thisUE.OCFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
						ikeLog.Errorf("Encrypting IKE message failed: %+v", err)
						thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
							thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
						cause := ngapType.Cause{
...
func parseIPAddressInformationToChildSecurityAssociation(
	...
	childSecurityAssociation.LocalPublicIPAddr = net.ParseIP(context.OCFSelf().IKEBindAddress)
