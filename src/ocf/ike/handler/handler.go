...
func HandleIKESAINIT(udpConn *net.UDPConn, ocfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	ikeSecurityAssociation := ocfSelf.NewIKESecurityAssociation()
	...
	idPayload := []ike_message.IKEPayloadType{
		ike_message.BuildIdentificationResponder(ike_message.ID_FQDN, []byte(ocfSelf.FQDN)),
	}
...
func HandleIKEAUTH(udpConn *net.UDPConn, ocfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	ikeSecurityAssociation, ok := ocfSelf.IKESALoad(localSPI)
	if !ok {
	...
	case PreSignalling:
		...
		if certificateRequest != nil {
			ikeLog.Info("UE request CCF certificate")
			if CompareRootCertificate(certificateRequest.CertificateEncoding, certificateRequest.CertificationAuthority) {
				responseCertificate =
					ike_message.BuildCertificate(ike_message.X509CertificateSignature, ocfSelf.CCFCertificate)
			}
		}
		...
		if securityAssociation != nil {
			...
			for _, proposal := range securityAssociation.Proposals {
				...
				if _, ok := ocfSelf.ChildSA.Load(spi); ok {
					continue
				}
		...
		// Identification
		responseIdentification = ike_message.BuildIdentificationResponder(ike_message.ID_FQDN, []byte(ocfSelf.FQDN))
		ikePayload = append(ikePayload, responseIdentification)

		// Certificate
		if responseCertificate == nil {
			responseCertificate = ike_message.BuildCertificate(ike_message.X509CertificateSignature, ocfSelf.CCFCertificate)
		}
		...
		signedAuth, err := rsa.SignPKCS1v15(rand.Reader, ocfSelf.CCFPrivateKey, crypto.SHA1, sha1HashFunction.Sum(nil))
		if err != nil {
	...
	case EAPSignalling:
		// If success, CCF will send an UPLinkNASTransport to AMF
		if eap != nil {
			...
			if anParameters != nil {
				...
				ue.CCFIKESecurityAssociation = ikeSecurityAssociation
				ue.AMF = selectedAMF
	...
	case PostSignalling:
		...
		if authentication != nil {
			...
			pseudorandomFunction, ok := NewPseudorandomFunction(thisUE.Kocf, transformPseudorandomFunction.TransformID)
			if !ok {
				ikeLog.Error("[IKE] Get an unsupported pseudorandom funcion. This may imply an unsupported transform is chosen.")
				return
			}
			...
			// TODO: Finish authentication test for UE and CCF
			/*
		...
		if addrRequest {
			...
			// IP addresses (IPSec)
			ueIPAddr := ocfSelf.NewInternalUEIPAddr(thisUE)
			...
			attributes = append(attributes, ike_message.BuildConfigurationAttribute(
				ike_message.INTERNAL_IP4_NETMASK, ocfSelf.Subnet.Mask))
		...
		responseNotification = ike_message.BuildNotifyNAS_IP4_ADDRESS(ocfSelf.IPSecGatewayAddress)
		ikePayload = append(ikePayload, responseNotification)

		// Notification(NSA_TCP_PORT)
		responseNotification = ike_message.BuildNotifyNAS_TCP_PORT(ocfSelf.TCPPort)
		ikePayload = append(ikePayload, responseNotification)
		...
		if thisUE.TemporaryPDUSessionSetupData != nil {
			for {
				if len(thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
					...
					ikeSecurityAssociation := thisUE.CCFIKESecurityAssociation
					...
					if err := EncryptProcedure(thisUE.CCFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
						ikeLog.Errorf("Encrypting IKE message failed: %+v", err)
						thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
							thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
						cause := ngapType.Cause{
...
func HandleCREATECHILDSA(udpConn *net.UDPConn, ocfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
...
func parseIPAddressInformationToChildSecurityAssociation(
	...
	childSecurityAssociation.LocalPublicIPAddr = net.ParseIP(context.CCFSelf().IKEBindAddress)
