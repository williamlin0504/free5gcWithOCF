...
type CCFUe struct {
	...
	// UE setup NAS TCP connection with CCF
	...
	CCFIKESecurityAssociation   *IKESecurityAssociation
	CCFChildSecurityAssociation *ChildSecurityAssociation
...
func (ue *CCFUe) init(ranUeNgapId int64) {
	ue.RanUeNgapId = ranUeNgapId
	ue.AmfUENGAPID = AmfUENGAPIDUnspecified
	ue.PduSessionList = make(map[int64]*PDUSession)
}

func (ue *CCFUe) Remove() {
	// remove from AMF context
	ue.DetachAMF()
	// remove from CCF context
	ocfSelf := CCFSelf()
	ocfSelf.DeleteCcfUe(ue.RanUeNgapId)
	ocfSelf.DeleteIKESecurityAssociation(ue.CCFIKESecurityAssociation.LocalSPI)
...
func (ue *CCFUe) FindPDUSession(pduSessionID int64) *PDUSession {
...
func (ue *CCFUe) CreatePDUSession(pduSessionID int64, snssai ngapType.SNSSAI) (*PDUSession, error) {
...
func (ue *CCFUe) CreateIKEChildSecurityAssociation(
...
func (ue *CCFUe) DetachAMF() {
	if ue.AMF == nil {
		return
	}
	delete(ue.AMF.CcfUeList, ue.RanUeNgapId)
}
