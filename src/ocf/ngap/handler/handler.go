...
func HandleNGSetupResponse(sctpAddr string, conn *sctp.SCTPConn, message *ngapType.NGAPPDU) {
	...
	for _, ie := range ngSetupResponse.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDAMFName:
			ngapLog.Traceln("[NGAP] Decode IE CCFName")
			amfName = ie.Value.AMFName
			if amfName == nil {
				ngapLog.Errorf("CCFName is nil")
	...
	if len(iesCriticalityDiagnostics.List) != 0 {
		ngapLog.Traceln("[NGAP] Sending error indication to CCF, because some mandatory IEs were not included")
	...
	amfInfo := ocfSelf.NewCcfCcf(sctpAddr, conn)

	if amfName != nil {
		amfInfo.CCFName = amfName
	}
	...
	if relativeCCFCapacity != nil {
		amfInfo.RelativeCCFCapacity = relativeCCFCapacity
	}
...
func HandleNGSetupFailure(sctpAddr string, conn *sctp.SCTPConn, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle NG Setup Failure")
...
func HandleNGReset(amf *context.CCFAMF, message *ngapType.NGAPPDU) {

	ngapLog.Infoln("[CCF] Handle NG Reset")
	...
	case ngapType.ResetTypePresentPartOfNGInterface:
		...
		for _, ueAssociatedLogicalNGConnectionItem := range partOfNGInterface.List {
			if ueAssociatedLogicalNGConnectionItem.RANUENGAPID != nil {
				...
				ngapLog.Tracef("CcfUeNgapID[%d]", ueAssociatedLogicalNGConnectionItem.AmfUENGAPID.Value)
				ue = amf.FindUeByCcfUeNgapID(ueAssociatedLogicalNGConnectionItem.AmfUENGAPID.Value)
			}

			if ue == nil {
				ngapLog.Warn("Cannot not find UE Context")
				if ueAssociatedLogicalNGConnectionItem.AmfUENGAPID != nil {
					ngapLog.Warnf("CcfUeNgapID[%d]", ueAssociatedLogicalNGConnectionItem.AmfUENGAPID.Value)
				}
				if ueAssociatedLogicalNGConnectionItem.RANUENGAPID != nil {
...
func HandleNGResetAcknowledge(amf *context.CCF, message *ngapType.NGAPPDU) {

	ngapLog.Infoln("[CCF] Handle NG Reset Acknowledge")

	var uEAssociatedLogicalNGConnectionList *ngapType.UEAssociatedLogicalNGConnectionList
	var criticalityDiagnostics *ngapType.CriticalityDiagnostics

	if amf == nil {
		ngapLog.Error("CCF Context is nil")
		return
	}
	...
	if uEAssociatedLogicalNGConnectionList != nil {
		...
		for i, item := range uEAssociatedLogicalNGConnectionList.List {
			if item.AmfUENGAPID != nil && item.RANUENGAPID != nil {
				ngapLog.Tracef("%d: CcfUeNgapID[%d] RanUeNgapID[%d]",
					i+1, item.AmfUENGAPID.Value, item.RANUENGAPID.Value)
			} else if item.AmfUENGAPID != nil {
				ngapLog.Tracef("%d: CcfUeNgapID[%d] RanUeNgapID[unknown]", i+1, item.AmfUENGAPID.Value)
			} else if item.RANUENGAPID != nil {
				ngapLog.Tracef("%d: CcfUeNgapID[unknown] RanUeNgapID[%d]", i+1, item.RANUENGAPID.Value)
			}
		}
	}
...
func HandleInitialContextSetupRequest(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Initial Context Setup Request")

	var AmfUENGAPID *ngapType.AmfUENGAPID
	var ranUeNgapID *ngapType.RANUENGAPID
	var oldCCF *ngapType.CCFName
	...
	var ocfUe *context.CCFUe
	var ocfSelf = context.CCFSelf()
	...
	for _, ie := range initialContextSetupRequest.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDOldCCF:
			ngapLog.Traceln("[NGAP] Decode IE OldCCF")
			oldCCF = ie.Value.OldCCF
	...
	if len(iesCriticalityDiagnostics.List) > 0 {
		ngapLog.Traceln("[NGAP] Sending unsuccessful outcome to CCF, because some mandatory IEs were not included")
		...
		ngap_message.SendInitialContextSetupFailure(amf, ocfUe, *cause, failedListCxtFail, &criticalityDiagnostics)
		return
	}
	...
	if (AmfUENGAPID != nil) && (ranUeNgapID != nil) {
		// Find UE context
		var ok bool
		ocfUe, ok = ocfSelf.UePoolLoad(ranUeNgapID.Value)
		if !ok {
			...
			if ocfUe.AmfUENGAPID != AmfUENGAPID.Value {
				// TODO: build cause and handle error
	...
	ocfUe.AmfUENGAPID = AmfUENGAPID.Value
	ocfUe.RanUeNgapId = ranUeNgapID.Value

	if pduSessionResourceSetupListCxtReq != nil {
		if ueAggregateMaximumBitRate != nil {
			ocfUe.Ambr = ueAggregateMaximumBitRate
		} else {
		...
		ocfUe.TemporaryPDUSessionSetupData = &context.PDUSessionSetupTemporaryData{
			SetupListCxtRes:  setupListCxtRes,
			FailedListCxtRes: failedListCxtRes,
		}
		ocfUe.TemporaryPDUSessionSetupData.NGAPProcedureCode.Value =
		...
		for _, item := range pduSessionResourceSetupListCxtReq.List {
			...
			pduSession, err := ocfUe.CreatePDUSession(pduSessionID, snssai)
			if err != nil {
			...
			success, resTransfer := handlePDUSessionResourceSetupRequestTransfer(ocfUe, pduSession, transfer)
			if success {
				// Append this PDU session to unactivated PDU session list
				ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
					append(ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession, pduSessionID)
			} else {
				// Delete the pdusession store in UE conext
				delete(ocfUe.PduSessionList, pduSessionID)
	...
	if allowedNSSAI != nil {
		ocfUe.AllowedNssai = allowedNSSAI
	}

	if maskedIMEISV != nil {
		ocfUe.MaskedIMEISV = maskedIMEISV
	}

	if ueRadioCapability != nil {
		ocfUe.RadioCapability = ueRadioCapability
	}

	if coreNetworkAssistanceInformation != nil {
		ocfUe.CoreNetworkAssistanceInformation = coreNetworkAssistanceInformation
	}

	if indexToRFSP != nil {
		ocfUe.IndexToRfsp = indexToRFSP.Value
	}

	if ueSecurityCapabilities != nil {
		ocfUe.SecurityCapabilities = ueSecurityCapabilities
	}

	if securityKey != nil {
		ocfUe.Kocf = securityKey.Value.Bytes
	}
	...
	ikeSecurityAssociation := ocfUe.CCFIKESecurityAssociation
	...
	ocfUe.CCFIKESecurityAssociation.State++

	// Send IKE message to UE
	handler.SendIKEMessageToUE(ocfUe.IKEConnection.Conn, ocfUe.IKEConnection.CCFAddr,
		ocfUe.IKEConnection.UEAddr, responseIKEMessage)
}
...
func handlePDUSessionResourceSetupRequestTransfer(ue *context.CCFUe, pduSession *context.PDUSession,
	...
	if upfIPv4 != "" {
		...
		if userPlaneConnection, ok := ocfSelf.GTPConnectionWithUPFLoad(upfIPv4); ok {
			// UPF UDP address
			...
			// Store GTP connection with UPF into CCF context
			ocfSelf.GTPConnectionWithUPFStore(upfIPv4, userPlaneConnection)
		}
...
func HandleUEContextModificationRequest(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle UE Context Modification Request")

	if amf == nil {
		ngapLog.Error("Corresponding CCF context not found")
		return
	}

	var AmfUENGAPID *ngapType.AmfUENGAPID
	var newCcfUeNgapID *ngapType.AmfUENGAPID
	var ranUeNgapID *ngapType.RANUENGAPID
	var ueAggregateMaximumBitRate *ngapType.UEAggregateMaximumBitRate
	var ueSecurityCapabilities *ngapType.UESecurityCapabilities
	var securityKey *ngapType.SecurityKey
	...
	for _, ie := range ueContextModificationRequest.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDNewAmfUENGAPID:
			...
			newCcfUeNgapID = ie.Value.NewAmfUENGAPID
		}
	}
	...
	if newCcfUeNgapID != nil {
		ngapLog.Debugf("New CcfUeNgapID[%d]\n", newCcfUeNgapID.Value)
		ocfUe.AmfUENGAPID = newCcfUeNgapID.Value
	}
...
func HandleUEContextReleaseCommand(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle UE Context Release Command")
	...
	case ngapType.UENGAPIDsPresentUENGAPIDPair:
		...
		if !ok {
			ocfUe = amf.FindUeByCcfUeNgapID(ueNgapIDs.UENGAPIDPair.AmfUENGAPID.Value)
		}
	case ngapType.UENGAPIDsPresentAmfUENGAPID:
		// TODO: find UE according to specific CCF
		// The implementation here may have error when CCF need to
		// connect multiple CCFs.
		// Use UEpool in CCF context can solve this problem
		ocfUe = amf.FindUeByCcfUeNgapID(ueNgapIDs.AmfUENGAPID.Value)
	}
...
func HandleDownlinkNASTransport(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Downlink NAS Transport")
	...
	var ocfUe *context.CCFUe
	var ocfSelf *context.CCFContext = context.CCFSelf()
	...
	if AmfUENGAPID != nil {
		if ocfUe.AmfUENGAPID == context.AmfUENGAPIDUnspecified {
	...
	if nasPDU != nil {
		...
		if ocfUe.CCFChildSecurityAssociation == nil {
			var identifier uint8
			ikeSecurityAssociation := ocfUe.CCFIKESecurityAssociation

			for {
				identifier = uint8(rand.Uint32())
				if identifier != ocfUe.CCFIKESecurityAssociation.LastEAPIdentifier {
					ocfUe.CCFIKESecurityAssociation.LastEAPIdentifier = identifier
					break
				}
			}
			...
			for i := 0; i < maxRetryTimes; i++ {
				if ocfUe.TCPConnection == nil {
					if i == (maxRetryTimes - 1) {
						ngapLog.Warn(
							"No connection found for UE to send NAS message. This message will be cached in CCF")
						ocfUe.TemporaryCachedNASMessage = nasPDU.Value
						return
					} else {
			...
			// Send to UE
			if n, err := ocfUe.TCPConnection.Write(nasPDU.Value); err != nil {
...
func HandlePDUSessionResourceSetupRequest(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle PDU Session Resource Setup Request")
	...
	if len(iesCriticalityDiagnostics.List) > 0 {
		// TODO: Send error indication to CCF
		ngapLog.Errorln("Sending error indication to CCF")
		return
	}
	...
	if ocfUe.TemporaryPDUSessionSetupData != nil {
		for {
			if len(ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
				pduSessionID := ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[0]
				pduSession := ocfUe.PduSessionList[pduSessionID]
				...
				for {
					...
					if _, ok := ocfSelf.ChildSA.Load(uint32(randomUint64)); !ok {
				...
				if ok := ike_message.AppendTransformToProposal(proposal, encryptionTransform); !ok {
					...
					ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
						ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
					...
					ngap_message.AppendPDUSessionResourceFailedToSetupListSURes(
						ocfUe.TemporaryPDUSessionSetupData.FailedListSURes, pduSessionID, transfer)
					continue
				}
				// Integrity transform
				...
				// TSi
				ocfIPAddr := net.ParseIP(ocfSelf.IPSecGatewayAddress)
				individualTrafficSelector := ike_message.BuildIndividualTrafficSelector(
					...
					0, 65535, ocfIPAddr.To4(), ocfIPAddr.To4())
				...
				// TSr
				ueIPAddr := net.ParseIP(ocfUe.IPSecInnerIP)
				individualTrafficSelector = ike_message.BuildIndividualTrafficSelector(
					ike_message.TS_IPV4_ADDR_RANGE, ike_message.IPProtocolAll,
				...
				notifyUPIPAddr := ike_message.BuildNotifyUP_IP4_ADDRESS(ocfSelf.IPSecGatewayAddress)

				ikePayload = append(ikePayload, notifyUPIPAddr)

				if err := handler.EncryptProcedure(
					ocfUe.CCFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
					ngapLog.Errorf("Encrypting IKE message failed: %+v", err)
					ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
						ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
					cause := buildCause(ngapType.CausePresentTransport, ngapType.CauseTransportPresentTransportResourceUnavailable)
					...
					ngap_message.AppendPDUSessionResourceFailedToSetupListSURes(ocfUe.TemporaryPDUSessionSetupData.FailedListSURes,
				...
				// Send PDU Session Resource Setup Response to CCF
...
func HandlePDUSessionResourceModifyRequest(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle PDU Session Resource Modify Request")
	...
	if pduSessionResourceModifyListModReq != nil {
		...
		for _, item := range pduSessionResourceModifyListModReq.List {
			...
			if pduSession = ocfUe.FindPDUSession(pduSessionID); pduSession == nil {
	...
	ngap_message.SendPDUSessionResourceModifyResponse(amf, ocfUe, responseList, failedListModRes, nil)
}
...
func handlePDUSessionResourceModifyRequestTransfer(
	...
	ngapLog.Trace("[CCF] Handle PDU Session Resource Modify Request Transfer")
...
func HandlePDUSessionResourceModifyConfirm(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle PDU Session Resource Modify Confirm")
	...
	if rANUENGAPID != nil {
		var ok bool
		ue, ok = ocfSelf.UePoolLoad(rANUENGAPID.Value)
		if !ok {
			ngapLog.Errorf("Unknown local UE NGAP ID. RanUENGAPID: %d", rANUENGAPID.Value)
			return
		}
	...
	if AmfUENGAPID != nil {
		if ue != nil {
			...
			ue = amf.FindUeByCcfUeNgapID(AmfUENGAPID.Value)
			if ue == nil {
				ngapLog.Errorf("Inconsistent remote UE NGAP ID, AmfUENGAPID: %d, ue.AmfUENGAPID: %d",
					AmfUENGAPID.Value, ue.AmfUENGAPID)
				return
...
func HandlePDUSessionResourceReleaseCommand(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle PDU Session Resource Release Command")
	var AmfUENGAPID *ngapType.AmfUENGAPID
	var rANUENGAPID *ngapType.RANUENGAPID
...
func HandleErrorIndication(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Error Indication")
	...
	for _, ie := range errorIndication.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDAmfUENGAPID:
			AmfUENGAPID = ie.Value.AmfUENGAPID
			ngapLog.Trace("[NGAP] Decode IE CcfUeNgapID")
		case ngapType.ProtocolIEIDRANUENGAPID:
			rANUENGAPID = ie.Value.RANUENGAPID
	...
	if (AmfUENGAPID != nil) && (rANUENGAPID != nil) {
		...
		ngapLog.Warnf("CCF UE NGAP ID is defined, value = %d", AmfUENGAPID.Value)
...
func HandleUERadioCapabilityCheckRequest(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle UE Radio Capability Check Request")
	var AmfUENGAPID *ngapType.AmfUENGAPID
	var rANUENGAPID *ngapType.RANUENGAPID
	var uERadioCapability *ngapType.UERadioCapability
...
func HandleCCFConfigurationUpdate(amf *context.CCF, message *ngapType.NGAPPDU) {

	ngapLog.Infoln("[CCF] Handle CCF Configuration Updaet")

	var aMFName *ngapType.CCFName
	var servedGUAMIList *ngapType.ServedGUAMIList
	var relativeCCFCapacity *ngapType.RelativeCCFCapacity
	var pLMNSupportList *ngapType.PLMNSupportList
	var aMFTNLAssociationToAddList *ngapType.CCFTNLAssociationToAddList
	var aMFTNLAssociationToRemoveList *ngapType.CCFTNLAssociationToRemoveList
	var aMFTNLAssociationToUpdateList *ngapType.CCFTNLAssociationToUpdateList
	...
	aMFConfigurationUpdate := initiatingMessage.Value.CCFConfigurationUpdate
	...
	for _, ie := range aMFConfigurationUpdate.ProtocolIEs.List {
		switch ie.Id.Value {
		case ngapType.ProtocolIEIDCCFName:
			ngapLog.Traceln("[NGAP] Decode IE CCFName")
			aMFName = ie.Value.CCFName
		case ngapType.ProtocolIEIDServedGUAMIList:
			ngapLog.Traceln("[NGAP] Decode IE ServedGUAMIList")
			servedGUAMIList = ie.Value.ServedGUAMIList
		case ngapType.ProtocolIEIDRelativeCCFCapacity:
			ngapLog.Traceln("[NGAP] Decode IE RelativeCCFCapacity")
			relativeCCFCapacity = ie.Value.RelativeCCFCapacity
		case ngapType.ProtocolIEIDPLMNSupportList:
			ngapLog.Traceln("[NGAP] Decode IE PLMNSupportList")
			pLMNSupportList = ie.Value.PLMNSupportList
		case ngapType.ProtocolIEIDCCFTNLAssociationToAddList:
			ngapLog.Traceln("[NGAP] Decode IE CCFTNLAssociationToAddList")
			aMFTNLAssociationToAddList = ie.Value.CCFTNLAssociationToAddList
		case ngapType.ProtocolIEIDCCFTNLAssociationToRemoveList:
			ngapLog.Traceln("[NGAP] Decode IE CCFTNLAssociationToRemoveList")
			aMFTNLAssociationToRemoveList = ie.Value.CCFTNLAssociationToRemoveList
		case ngapType.ProtocolIEIDCCFTNLAssociationToUpdateList:
			ngapLog.Traceln("[NGAP] Decode IE CCFTNLAssociationToUpdateList")
			aMFTNLAssociationToUpdateList = ie.Value.CCFTNLAssociationToUpdateList
		}
	}
	...
	if relativeCCFCapacity != nil {
		amf.RelativeCCFCapacity = relativeCCFCapacity
	}
	...
	successList := []ngapType.CCFTNLAssociationSetupItem{}
	if aMFTNLAssociationToAddList != nil {
		// TODO: Establish TNL Association with CCF
		for _, item := range aMFTNLAssociationToAddList.List {
			tnlItem := amf.AddCCFTNLAssociationItem(item.CCFTNLAssociationAddress)
			...
			setupItem := ngapType.CCFTNLAssociationSetupItem{
				CCFTNLAssociationAddress: item.CCFTNLAssociationAddress,
			}
			successList = append(successList, setupItem)
		}
	}
	if aMFTNLAssociationToRemoveList != nil {
		// TODO: Remove TNL Association with CCF
		for _, item := range aMFTNLAssociationToRemoveList.List {
			amf.DeleteCCFTNLAssociationItem(item.CCFTNLAssociationAddress)
		}
	}
	if aMFTNLAssociationToUpdateList != nil {
		// TODO: Update TNL Association with CCF
		for _, item := range aMFTNLAssociationToUpdateList.List {
			tnlItem := amf.FindCCFTNLAssociationItem(item.CCFTNLAssociationAddress)
			if tnlItem == nil {
				continue
			}
	...
	ngap_message.SendCCFConfigurationUpdateAcknowledge(amf, setupList, nil, nil)
}

func HandleRANConfigurationUpdateAcknowledge(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle RAN Configuration Update Acknowledge")
...
func HandleRANConfigurationUpdateFailure(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle RAN Configuration Update Failure")
	...
	if waitingTime != 0 {
		ngapLog.Infof("Wait at lease  %ds to resend RAN Configuration Update to same CCF[%s]",
			waitingTime, amf.SCTPAddr)
		ocfSelf.CCFReInitAvailableListStore(amf.SCTPAddr, false)
		time.AfterFunc(time.Duration(waitingTime)*time.Second, func() {
			ngapLog.Infof("Re-send Ran Configuration Update Message when waiting time expired")
			ocfSelf.CCFReInitAvailableListStore(amf.SCTPAddr, true)
...
func HandleDownlinkRANConfigurationTransfer(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Downlink RAN Configuration Transfer")
}

func HandleDownlinkRANStatusTransfer(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Downlink RAN Status Transfer")
}

func HandleCCFStatusIndication(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle CCF Status Indication")
}

func HandleLocationReportingControl(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Location Reporting Control")
}

func HandleUETNLAReleaseRequest(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle UE TNLA Release Request")
}

func HandleOverloadStart(amf *context.CCF, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[CCF] Handle Overload Start")
	...
	for _, ie := range overloadStart.ProtocolIEs.List {
		switch ie.Id.Value {
		case ngapType.ProtocolIEIDCCFOverloadResponse:
			ngapLog.Traceln("[NGAP] Decode IE CCFOverloadResponse")
			aMFOverloadResponse = ie.Value.CCFOverloadResponse
		case ngapType.ProtocolIEIDCCFTrafficLoadReductionIndication:
			ngapLog.Traceln("[NGAP] Decode IE CCFTrafficLoadReductionIndication")
			aMFTrafficLoadReductionIndication = ie.Value.CCFTrafficLoadReductionIndication
...
func HandleOverloadStop(amf *context.CCF, message *ngapType.NGAPPDU) {

	ngapLog.Infoln("[CCF] Handle Overload Stop")
