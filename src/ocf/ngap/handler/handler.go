...
import (
	...
	gtp_service "free5gcWithOCFWithOCF/src/ocf/gtp/service"
	"free5gcWithOCFWithOCF/src/ocf/ike/handler"
...
func HandleNGSetupFailure(sctpAddr string, conn *sctp.SCTPConn, message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[OCF] Handle NG Setup Failure")
	...
	case ngapType.ResetTypePresentPartOfNGInterface:
		...
		for _, ueAssociatedLogicalNGConnectionItem := range partOfNGInterface.List {
			if ueAssociatedLogicalNGConnectionItem.RANUENGAPID != nil {
				...
				ue = amf.FindUeByOcfUeNgapID(ueAssociatedLogicalNGConnectionItem.OCFUENGAPID.Value)
			}
...
func HandleInitialContextSetupRequest(amf *context.OCFOCF, message *ngapType.NGAPPDU) {
	...
	if (amfUeNgapID != nil) && (ranUeNgapID != nil) {
		...
		if !ok {
			...
			if ocfUe.OcfUeNgapId != amfUeNgapID.Value {
				// TODO: build cause and handle error
	...
	ocfUe.OcfUeNgapId = amfUeNgapID.Value
	...
	ikeSecurityAssociation := ocfUe.OCFIKESecurityAssociation
	...
	ocfUe.OCFIKESecurityAssociation.State++

	// Send IKE message to UE
	handler.SendIKEMessageToUE(ocfUe.IKEConnection.Conn, ocfUe.IKEConnection.OCFAddr,
...
func handlePDUSessionResourceSetupRequestTransfer(ue *context.OCFUe, pduSession *context.PDUSession,
	...
	if upfIPv4 != "" {
		...
		if userPlaneConnection, ok := ocfSelf.GTPConnectionWithUPFLoad(upfIPv4); ok {
			...
			// Store GTP connection with UPF into OCF context
...
func HandleUEContextModificationRequest(amf *context.OCFOCF, message *ngapType.NGAPPDU) {
	...
	if newOcfUeNgapID != nil {
		...
		ocfUe.OcfUeNgapId = newOcfUeNgapID.Value
	}
...
func HandleUEContextReleaseCommand(amf *context.OCFOCF, message *ngapType.NGAPPDU) {
	...
	case ngapType.UENGAPIDsPresentUENGAPIDPair:
		...
		if !ok {
			ocfUe = amf.FindUeByOcfUeNgapID(ueNgapIDs.UENGAPIDPair.OCFUENGAPID.Value)
		}
	case ngapType.UENGAPIDsPresentOCFUENGAPID:
		...
		ocfUe = amf.FindUeByOcfUeNgapID(ueNgapIDs.OCFUENGAPID.Value)
	}
...
func HandleDownlinkNASTransport(amf *context.OCFOCF, message *ngapType.NGAPPDU) {
	...
	var ocfUe *context.OCFUe
	var ocfSelf *context.OCFContext = context.OCFSelf()
	...
	if amfUeNgapID != nil {
		if ocfUe.OcfUeNgapId == context.OcfUeNgapIdUnspecified {
	...
	if nasPDU != nil {
		...
		if ocfUe.OCFChildSecurityAssociation == nil {
			var identifier uint8
			ikeSecurityAssociation := ocfUe.OCFIKESecurityAssociation

			for {
				identifier = uint8(rand.Uint32())
				if identifier != ocfUe.OCFIKESecurityAssociation.LastEAPIdentifier {
					ocfUe.OCFIKESecurityAssociation.LastEAPIdentifier = identifier
					break
				}
			}
			...
			for i := 0; i < maxRetryTimes; i++ {
				if ocfUe.TCPConnection == nil {
					if i == (maxRetryTimes - 1) {
						ngapLog.Warn(
							"No connection found for UE to send NAS message. This message will be cached in OCF")
	...
	if ocfUe.TemporaryPDUSessionSetupData != nil {
		for {
			if len(ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
				...
				if err := handler.EncryptProcedure(
					ocfUe.OCFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
					ngapLog.Errorf("Encrypting IKE message failed: %+v", err)
					ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
						ocfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
					cause := buildCause(ngapType.CausePresentTransport, ngapType.CauseTransportPresentTransportResourceUnavailable)
...
func handlePDUSessionResourceModifyRequestTransfer(
	...
	ngapLog.Trace("[OCF] Handle PDU Session Resource Modify Request Transfer")
...
func HandleDownlinkRANConfigurationTransfer(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[OCF] Handle Downlink RAN Configuration Transfer")
}
...
func HandleLocationReportingControl(message *ngapType.NGAPPDU) {
	ngapLog.Infoln("[OCF] Handle Location Reporting Control")
}
