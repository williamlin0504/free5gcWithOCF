...
func (*UDM) Initialize(c *cli.Context) {
	...
	if config.udmcfg != "" {
		...
		DefaultUdmConfigPath := path_util.Gofree5gcWithOCFPath("free5gcWithOCF/config/udmcfg.conf")
...
func (udm *UDM) Start() {
	...
	udmLogPath := path_util.Gofree5gcWithOCFPath("free5gcWithOCF/udmsslkey.log")
	udmPemPath := path_util.Gofree5gcWithOCFPath("free5gcWithOCF/support/TLS/udm.pem")
	udmKeyPath := path_util.Gofree5gcWithOCFPath("free5gcWithOCF/support/TLS/udm.key")
	if sbi.Tls != nil {
		udmLogPath = path_util.Gofree5gcWithOCFPath(sbi.Tls.Log)
		udmPemPath = path_util.Gofree5gcWithOCFPath(sbi.Tls.Pem)
		udmKeyPath = path_util.Gofree5gcWithOCFPath(sbi.Tls.Key)
	}
