...
func (*SMF) Initialize(c *cli.Context) {
	...
	if config.smfcfg != "" {
		...
		DefaultSmfConfigPath := path_util.Go free5gcWithOCFPath(" free5gcWithOCF/config/smfcfg.conf")
	...
	if config.uerouting != "" {
		...
		DefaultUERoutingPath := path_util.Go free5gcWithOCFPath(" free5gcWithOCF/config/uerouting.yaml")
