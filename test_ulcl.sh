...
sudo ip link set  free5gc
sudo ip link set br-veth0 up
sudo ip link set br-veth0 master  free5gc
...
for i in $(seq -f "%02g" 1 $UPF_NUM); do
    ...
    sudo ip link set "br-veth${i}" master  free5gc
    ...
    cd src/upf/build && sudo -E ip netns exec "${UPFNS}${i}" ./bin/ free5gcf config/upfcfg.ulcl.yaml &
    sleep 1
done
