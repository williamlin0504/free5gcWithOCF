...
NF = $(GO_NF) $(C_NF)
GO_NF = amf ausf nrf nssf pcf ccf smf udm udr n3iwf
C_NF = upf
...
LDFLAGS = -X free5gcWithOCF/src/$(@F)/version.VERSION=$(VERSION) \
          -X free5gcWithOCF/src/$(@F)/version.BUILD_TIME=$(BUILD_TIME) \
          -X free5gcWithOCF/src/$(@F)/version.COMMIT_HASH=$(COMMIT_HASH) \
          -X free5gcWithOCF/src/$(@F)/version.COMMIT_TIME=$(COMMIT_TIME)
